    ...
    : wxFrame(NULL, wxID_ANY, "Aplicativo com Botão Giratório"), m_counter(0)
{
    ...
    // Título
    wxStaticText* title =
        new wxStaticText(panel, wxID_ANY, "Contador com Botão Giratório");
    ...
    // Seção do SpinButton
    wxStaticBoxSizer* spinButtonBox =
        new wxStaticBoxSizer(wxVERTICAL, panel, "Botão Giratório");
    ...
    wxStaticText* instructions = new wxStaticText(
        panel, wxID_ANY,
        "Use as setas do botão giratório para incrementar/decrementar o "
        "contador.\n"
...
void SpinFrame::OnSpinUp(wxSpinEvent& event)
{
    ...
    SetStatusText(
        wxString::Format("Contador incrementado para: %d", m_counter));
}
...
void SpinFrame::OnSpinDown(wxSpinEvent& event)
{
    ...
    m_counterText->SetLabel(wxString::Format("%d", m_counter));
    SetStatusText(
        wxString::Format("Contador decrementado para: %d", m_counter));
}
...
{
    wxMessageBox(
        "Aplicativo demonstrando o uso de controles giratórios em wxWidgets",
